...
def visualization(config, data, box_colors, box_color_map, apply_late_fusion, savename=None):
    ...
    for p in range(pred_len):
        ...
        for k in range(len(gt_max_iou_idx)):
            ...
            if p == 0:
                color = "g"
                plt.plot(corners[:, 0], corners[:, 1], c=color, linewidth=2, zorder=5)
                plt.scatter(c_x, c_y, s=5, c=color, zorder=5)
                ...
                plt.scatter(c_x, c_y, s=5, linewidth=2, c=color, zorder=5)
    vis_tag = data["vis_tag"]

    vis_color = {
        "ego_only" : (26, 67, 32), #green
        "consensus": (24, 55, 114, 161, 174, 201), #blue
        "original_fusion": (24, 55, 114, 161, 174, 201), #blue
        # "attacked_fusion": (64, 10, 90, 219, 151, 254), #violet
        "attacked_fusion": (60, 65, 70, 141, 153, 153), #grey
        # "upperbound": (120, 33, 45, 200, 56, 77) #red
        "upperbound": (128, 105, 19, 255, 188, 82)
    }

    bg_color = {
        "ego_only" : (248, 253, 253),
        "consensus": (247, 251, 247),
        "original_fusion": (247, 251, 247),
        # "attacked_fusion": (254, 247, 244),
        "attacked_fusion": (245, 250, 250),
        "upperbound": (255, 250, 247)
    }
    if vis_tag == "ego_only" or vis_tag == "lowerbound":
        # ego map visualization
        voxel = data["bev_seq"]
        maps = np.max(voxel, axis=-1)
        ...
        m1[m1 == 1] = vis_color[vis_tag][0] / 255
        m2[m2 == 1] = vis_color[vis_tag][1] / 255
        m3[m3 == 1] = vis_color[vis_tag][2] / 255

        m1[m1 == 0.99] = bg_color[vis_tag][0] / 255
        m2[m2 == 0.99] = bg_color[vis_tag][1] / 255
        m3[m3 == 0.99] = bg_color[vis_tag][2] / 255

    elif vis_tag == "original_fusion" or vis_tag =="attacked_fusion" or vis_tag == "upperbound" or vis_tag =="consensus":
        # teacher map visualization
        voxel = data["bev_seq_teacher"]
        maps = np.max(voxel, axis=-1)
        m = np.stack([maps, maps, maps], axis=-1)
        m[m == 0] = 0.99
        ...
        m1[m1 == 1] = vis_color[vis_tag][3] / 255
        m2[m2 == 1] = vis_color[vis_tag][4] / 255
        m3[m3 == 1] = vis_color[vis_tag][5] / 255

        
        # ego map visualization
        voxel_ego = data["bev_seq"]
        maps_ego = np.max(voxel_ego, axis=-1)
        m_ego = np.stack([maps_ego, maps_ego, maps_ego], axis=-1)
        m_ego[m_ego == 0] = 0.99
        ...
        m1[m1_ego == 1] = vis_color[vis_tag][0] / 255
        m2[m2_ego == 1] = vis_color[vis_tag][1] / 255
        m3[m3_ego == 1] = vis_color[vis_tag][2] / 255
    ...
    plt.imshow(maps, zorder=1)
    plt.imshow
    # ax = plt.gca()
    # ax.set_facecolor((bg_color[vis_tag][0]/255, bg_color[vis_tag][1]/255, bg_color[vis_tag][2]/255))
